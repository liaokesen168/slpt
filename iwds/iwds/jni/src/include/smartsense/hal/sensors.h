...
/**
 * IWDS smart sense module name
 */

#define SMART_SENSE_MODULE_NAME "Ingenic Sensor Module With IWDS Support"
...
/**
 * Name of the sensors device to open for iwds
 */
#define SENSORS_HARDWARE_POLL_FOR_IWDS       "poll_for_iwds"

#define SMART_SENSE_TYPE_BASE   (100)
...
#define SENSOR_TYPE_EKG                             (1 + SMART_SENSE_TYPE_BASE)

#define SENSOR_TYPE_UV                              (2 + SMART_SENSE_TYPE_BASE)

#define SENSOR_TYPE_GESTURE                         (3 + SMART_SENSE_TYPE_BASE)

#define SENSOR_TYPE_MOTION                          (4 + SMART_SENSE_TYPE_BASE)

#define SENSOR_TYPE_VOICE_TRIGGER                   (5 + SMART_SENSE_TYPE_BASE)
...
/**
 * status of heart rate sensor
 */
#define HEART_RATE_STATUS_UNAVALIABLE          1
#define HEART_RATE_STATUS_AVALIABLE            2
...
typedef struct sensors_event_t {
    ...
    union {
        ...
        /* relative heart rate wave from device in points*/
        float           relative_heart;
...
struct sensors_poll_device_t {
    ...
    /**
     * Add this function for AKM compass sensor
     */
    int (*calibrate)(struct sensors_poll_device_t *dev, int handle, int *data);

    /**
     * Add this function for Frizz gesture sensor to set on which hand the device is worn
     */
    int (*setRightHand)(struct sensors_poll_device_t *dev, int handle, int isRightHand);
...
static inline int sensors_open_for_iwds(const struct hw_module_t* module,
        struct sensors_poll_device_t** device) {
    return module->methods->open(module,
            SENSORS_HARDWARE_POLL_FOR_IWDS, (struct hw_device_t**)device);
}
