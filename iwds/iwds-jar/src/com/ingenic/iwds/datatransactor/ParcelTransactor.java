...
/**
 * 继承 {@link com.ingenic.iwds.datatransactor.DataTransactor DataTransactor}
 * 针对 Parcelable 对象的传输进行了优化, 传输效率优于
 * {@link com.ingenic.iwds.datatransactor.DataTransactor DataTransactor},
 * 同时兼容 {@link com.ingenic.iwds.datatransactor.DataTransactor DataTransactor}
 * 支持的所有类型数据的传输. 注意此类只支持单类型的 Parcelable 对象传输, 如果需要传输多种类型
 * 的对象, 请构造多个 ParcelTransactor.
 *
 * @param <T>
 *        实现 Parcelable 接口的类
 */
public class ParcelTransactor<T extends Parcelable> extends DataTransactor {
     ...
     */
    public ParcelTransactor(Context context, Creator<T> creator,
            DataTransactorCallback callback, String uuid) {
        super(context, callback, uuid);
        m_parcelableCreator = (Creator<Parcelable>) creator;
    }
